...
public class GoLogic {
	...
	// used by the recursive function to check if a section is captured
	...
	public MoveResult playMove(String color, String move, String capture) {
		if (color == null || !(color.equals("white") || color.equals("black")) || move == null || move.isEmpty()) {
		...
		if (move.toUpperCase(Locale.ENGLISH).equals("PASS")) {
			...
			if (this.gameInstance.numPasses < 2) {
				...
				// this is the third pass, end the game
				// compute scores
				computeScores();
			...
			// move is set on board, now check for capture
	...
	// used to keep track of which empty spaces we already checked for control
	...
	private void recursiveCaptureCheck(String checkPosition, String colorToCapture) {
		...
		if (colorAtPosition.equals(colorToCapture)) {
			// add this to potential captures
			potentialCaptures.add(checkPosition);
			// recursively check pieces around this one
			String top = getPositionAbove(checkPosition);
			...
			// the color at this position is a different color than the one we are trying to
			// capture,
			// stop recursion down this path
			return;
		}
	}
